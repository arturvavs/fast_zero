    ...
    if db_user:
        if db_user.username == user.username:
            raise HTTPException(
                ...
                detail='Nome de usuário informado já está em uso',
            )
        elif db_user.email == user.email:
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST,
                detail='Email informado já está em uso',
            )
    db_user = User(
...
def update_user(
    ...
    if current_user.id != user_id:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST, detail='Sem privilégios suficientes'
        )
    current_user.email = user.email
    current_user.username = user.username
    current_user.password = get_password_hash(user.username)
    session.commit()
